...
fn update_ik(
	...
	let mut func = || -> color_eyre::Result<()> {
		...
		for i in 0..SKELETON_ARR_LEN {
			...
			skeleton_root_array[i] =
				Some(transforms.get(entity)?.1.reparented_to(root));
			if test {
			...
			match skeleton_root_array[i] {
				Some(t) => {
					if t.translation.x.is_nan()
						|| t.translation.y.is_nan()
						|| t.translation.z.is_nan()
					{
			...
			match headset_input {
				Ok(input) => {
					if (input.0.location_flags.into_raw() & 0b1111) ^ 0b1111 == 0 {
						...
						let head_offset = (skeleton.left.eye.translation
							+ skeleton.right.eye.translation)
							/ 2.;
		...
		let final_left_hand = if testing_input_override {
			Transform::IDENTITY
				.with_translation(Vec3::new(-0.4, 0.8, -0.4) * height_factor)
		} else {
			match left_controller_input {
				Ok(input) => {
					if (input.0.location_flags.into_raw() & 0b1111) ^ 0b1111 == 0 {
						Transform {
							translation: input.0.pose.position.to_vec3()
								* height_factor,
							rotation: input.0.pose.orientation.to_quat(),
							scale: skeleton.left.hand.scale,
						}
					} else {
						root_skeleton.left.hand.with_rotation(
							flip_quat_l * root_skeleton.left.hand.rotation,
						)
					}
				}
		...
		let final_right_hand = if testing_input_override {
			Transform::IDENTITY
				.with_translation(Vec3::new(0., 0.8, -0.4) * height_factor)
		} else {
			match right_controller_input {
				Ok(input) => {
					if (input.0.location_flags.into_raw() & 0b1111) ^ 0b1111 == 0 {
						Transform {
							translation: input.0.pose.position.to_vec3()
						...
						root_skeleton.right.hand.with_rotation(
							flip_quat_r * root_skeleton.right.hand.rotation,
						)
					}
				}
		...
		if new_hip_rot.x.is_nan() {
			panic!("Calculating the hip rotation from euler failed, created NaN")
		};
