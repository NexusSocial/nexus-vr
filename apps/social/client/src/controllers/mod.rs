...
use bevy::prelude::{GlobalTransform, Mut};
...
use social_networking::data_model::{Local, PlayerPose};
...
fn drive_input_pose(
	...
	let mut tracking_root = match tracking_root.get_single() {
		Ok(tracking_root) => tracking_root,
		Err(_) => return,
	};
	let (oculus_controller, frame_state, xr_input) = match (oculus_controller, frame_state, xr_input) {
		(Some(oculus_controller), Some(frame_state), Some(xr_input)) => (oculus_controller, frame_state, xr_input),
		(_, _, _) => return,
	};
