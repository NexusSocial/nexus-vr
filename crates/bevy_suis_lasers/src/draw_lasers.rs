...
fn setup(
	...
	commands.spawn((
		Laser {
			...
			end: Vec3::new(2.0, 2.5, 1.0),
			current_end: Vec3::new(2.0, 0.5, 1.0),
		},
...
fn update_laser(
    ...
    for (entity, mut laser) in laser_query.iter_mut() {
        ...
        // Update current_end
        laser.current_end = laser
            .current_end
            .lerp(laser.end, time.delta_seconds() * INTERPOLATION_SPEED);

        // Calculate length
        let length = laser.current_end.distance(laser.start);
        ...
        let laser_direction = (laser.current_end - laser.start).normalize();
        ...
        while position_along < length {
            ...
            let segment_position = laser.start + laser_direction * (position_along + segment_length / 2.0);
